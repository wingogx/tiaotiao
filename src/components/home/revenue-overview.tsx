...
export function RevenueOverview({ data }: { data: DashboardData }) {
  ...
  return (
    <section className="page-shell grid gap-6 lg:grid-cols-[1.15fr_0.85fr]">
      <Card className="legacy-card-hover rounded-[28px] p-7">
            ...
            return (
              <div key={item.date} className="flex h-full flex-col justify-end gap-2">
                <div className="relative flex-1 overflow-hidden rounded-t-[18px] rounded-b-[10px] bg-[rgba(19,51,40,0.08)]">
                  <div
                    className={`absolute inset-x-0 bottom-0 rounded-t-[18px] ${positive ? 'legacy-gradient' : 'bg-[linear-gradient(180deg,#f59e0b,#d04545)]'}`}
              ...
              return (
                ...
                >
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <div className={`flex items-center gap-2 text-sm font-semibold ${positive ? 'text-[var(--accent)]' : 'text-[var(--danger)]'}`}>
                      {positive ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
                      {formatCurrency(project.total)}
                    </div>
                  </div>
                  <div className="legacy-progress mt-4">
                    <span style={{ width: `${Math.min(Math.abs(project.total) / 100000 * 100, 100)}%` }} />
                  </div>
                </div>
              );
            })
